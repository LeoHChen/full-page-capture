...
import {saveCapture} from './store.mjs';
let busy = false;
chrome.runtime.onMessage.addListener((message, sender, respond) => {
  if (sender.id !== chrome.runtime.id || sender.url !== chrome.runtime.getURL('popup.html') || message.type !== 'capture') return;
  if (busy) { respond({ok: false, error: 'A capture is already running. Please wait.'}); return; }
  if (!Number.isInteger(message.tabId)) { respond({ok: false, error: 'Select a webpage first.'}); return; }
  busy = true;
  run(message).finally(() => {busy = false;});
  respond({ok: true});
});
async function run(message) {
  ...
  try {
    ...
    const shot = await capturePage(chrome, message.tabId, {preload: message.preload === true});
  ...
  } finally { await chrome.action.setBadgeText({text: ''}).catch(() => {}); }
}
